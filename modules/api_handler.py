...
from datetime import datetime  # Corrigindo o import
from config import OAUTH_CONFIG, MEDIAWIKI_CONFIG
from utils.logger import logger  # Certifique-se de que logger seja uma instância válida
...
def realizar_edicao(page_title, post_content, token, oauth, nome_arquivo):
    ...
    if response.status_code == 200:
        ...
        print(f'[{data_hora_atual}] ({nome_arquivo}) {mensagem}')
        logger.error(f'[{data_hora_atual}] ({nome_arquivo}) {mensagem}')
